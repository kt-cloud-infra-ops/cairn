---
name: ops-incident
description: "운영 장애 인지 → 임시 회피 → 원인 분석 → 보고서(CINM/Confluence) → 후속 운영개선 태스크 생성 + hotfix handoff까지 표준화. harness-service-ops branch의 운영 장애 sub-owner."
---
...
1. **장애 신고 수집** — 슬랙 / 알람 / CINM 번호 / 외부 운영팀 신고
2. **시스템 식별** — 어느 서비스 (luppiter / gaia / cmdb / infraops-* 등), 어느 컴포넌트 (scheduler / web / DB)
3. **등급 추정** — 1~4등급 / 관리장애 / 이상징후 (KT cloud 사내 장애 대응 프로세스 참조: `references/incident-response-process-kt-cloud.md`)
4. **영향 범위 식별** — 채널/서비스 한정 여부, 고객 영향 여부, VOC 발생 가능성

#### [GATE 0] 분류 확정
- [ ] 장애 등급 사용자 명시 확인 또는 `[미확인]` 마킹
- [ ] 영향 범위 확인 (서비스명, 채널, 내부/외부 영향)
- [ ] CINM 인시던트 번호 (또는 발급 필요 여부)
- [ ] 서비스 분류 — 인프라(luppiter/gaia/cmdb/hermes/hera 등) vs devops(infraops-*) — Phase 4 프로젝트 분기 결정용
...
8. **회피 기록** — 시각 + 액션 + 책임자 기록 (작업일지 + CINM description)

### Phase 2: 원인 분석

9. **로그/DB 흔적 조사** — 운영 PG 로그, `x00_batch_event_log`, Zabbix events/alerts/auditlog 등
...
14. **Confluence 페이지 작성** — 5섹션 flat (`references/confluence-template.md`)
    - 위치: 사용자가 사전에 생성한 Confluence 페이지 (있으면 그 페이지 ID 사용) 또는 CL23 스페이스 신규 생성
    - 양식: 1.기본정보 / 2.원인&영향도 / 3.조치 / 4.Timetable(5컬럼) / 5.재발방지

15. **CINM description ADF append** — 8섹션 (`references/cinm-template.md`)
...
| 서비스 분류 | Jira 프로젝트 | 비고 |
|-----------|-------------|------|
| 인프라 (luppiter, gaia, cmdb, hermes, hera, tech-sso, infrafe) | **TECHIOPS26** | 접두사 파싱 — `agents/rules/service-mapping.md` |
| devops (infraops-batch, infraops-api, infraops-bff, infraops-frontend, infraops-message-bridge, infraops-jira-bot) | **별도 devops 프로젝트** | 정확한 키는 사용자 확인 필요 — 미확정 시 사용자 질문 |

태스크 내용:
- summary: `[CINM-{N} 후속] {root cause 요약}` (예: `[CINM-149 후속] luppiter_scheduler CSV escape 정합화`)
- description: 사고 요약 + root cause + 재발 방지 대책 (보고서 링크 포함)
- parent: 해당 서비스 운영 에픽 (예: TECHIOPS26-482 유피테르 운영 업무)
- assignee: 복구반장 또는 사용자 확인
- 시작일/기한: 재발 방지 대책 배포 일정 기준

17. **hotfix Jira 티켓 생성** (필요 시) — TECHIOPS26-{N} (인프라) 또는 devops 프로젝트
...
18. **`harness-dev-process` handoff** — hotfix 구현은 dev branch에서 진행
    - handoff 시 전달: Jira 티켓 번호, CINM 번호, root cause, 재발 방지 대책, 배포 일정

#### [GATE 3] SHIP
- [ ] Confluence 보고서 URL 확인
- [ ] CINM description 업데이트 확인 (8섹션 완료)
- [ ] 운영개선 태스크 번호 + 프로젝트 (TECHIOPS26 또는 devops)
...
- [ ] [CONTENT] CINM description 8섹션 완료
- [ ] [CONTENT] 운영개선 Jira 태스크 번호 (TECHIOPS26 또는 devops)
- [ ] [GATE] GATE 0~3 모두 통과
...
- 운영팀(Cloud통합관제팀, Cloud서비스운영팀 등)이 CINM 인시던트 생성 시 운영팀 작성한 description의 타임라인이 권위 있는 기록 — 본 스킬은 그 description에 append 형태로 추가

## 관련 문서

- `references/confluence-template.md` — 5섹션 flat 표준 양식 (팀장님 스타일 기반)
- `references/cinm-template.md` — CINM 8섹션 ADF 표준 양식
- `references/incident-response-process-kt-cloud.md` — KT cloud 사내 장애 대응 프로세스
- `references/incident-report-sample-cases.md` — 25.05.04 / 26.05.21 실제 사례
- `agents/rules/service-mapping.md` — 서비스 ↔ Jira 프로젝트 매핑
- `agents/skills/jira-rest-ops/` — Jira REST API 헬퍼 (CINM description append, 태스크 생성)
...
2026-05-21 ES0002 HW Zabbix 적재 실패 사고 처리 시 `harness-orchestrator` 가 "Jira/코드 수정 키워드" 우선 매칭으로 `dev` branch에 잘못 라우팅됨. 운영 장애 보고서 프로세스가 별도 흐름임에도 개발 오케스트레이터(Phase 0~5)로 진입 → 사용자 정정 후 모드 전환.
