...
### 사용자 선택 — TUI 우선 (CRITICAL)

사용자에게 **선택지·분기·확인**을 물을 때는 자유 텍스트 나열 대신 **도구의 structured choice(선택 TUI)를 기본으로 사용**한다.

- **Claude Code**: `AskUserQuestion` 도구로 선택지 제시 (라벨 + 설명, 추천안을 첫 옵션에 "(추천)" 표기)
- **Codex / 기타**: 그 도구의 선택 메뉴(numbered choice/TUI) 사용
- 적용 대상: 방향 결정, 옵션 비교, 설치/clone 여부, branch 선택, 다음 단계 등 **답이 갈리는 모든 지점**
- 예외: 단순 yes/no 확인, 자유 서술이 필요한 질문, 자동 진행 가능한 경우(CRITICAL 아님)
- 이유: 선택이 명확히 보이고, 도구·세션 간 일관되며, 사용자 응답 비용이 낮다
